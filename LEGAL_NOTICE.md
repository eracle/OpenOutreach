...
- **Profiling for marketing (profile vector).** Contributed profile vectors additionally feed a profiling product that learns which professional-profile characteristics correlate with response, to target B2B outreach. This is **profiling** (Art. 4(4) GDPR) and a further processing purpose beyond resolution, operating only on the store's non-EU/EEA/UK/CH professional contacts. <!-- TODO(lawyer): this purpose is gated on the rewritten LIA + the controller-relocation/sending-regime questions in roadmap p1-e3 and must not be relied on until those clear. -->
