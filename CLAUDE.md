...
- **`pipeline/`** — Candidate sourcing, qualification, and pool management:
  ...
  - `pools.py` — Pool management via composable generators. Three generators chain via `next(upstream, None)`: `search_source(session)` yields keywords from `search_one()`, `qualify_source(session, qualifier)` yields public_ids from `qualify_one()` (checks `_positive_pool_empty()` to do ONE search before qualifying in exploit mode when no P > 0.5 candidates exist), `ready_source(session, qualifier, pipeline)` yields ready-to-connect candidates by checking `get_ready_candidate()` → `promote_to_ready()` → pulling from `qualify_source`. Each `qualify_source` iteration produces exactly one label, preventing infinite-search-without-qualifying. `_positive_pool_empty(qualifier, candidates)` returns True only in exploit mode (n_neg > n_pos) when no candidate has P > 0.5; returns False on cold start, explore mode, or empty candidates. `get_candidate(session, qualifier, pipeline=None, is_partner=False)` — partner path: queries `get_qualified_profiles()` directly, ranks, returns top (bypasses READY_TO_CONNECT). Regular path: `next(ready_source(...), None)`.
