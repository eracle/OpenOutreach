...
4. **Legal notice** — `_require_legal_acceptance(campaign)` displays GitHub URL to `LEGAL_NOTICE.md`, prompts for acceptance (y/n) per campaign. Acceptance is stored as `Campaign.legal_accepted` (BooleanField). On startup, all campaigns with `legal_accepted=False` are prompted.
