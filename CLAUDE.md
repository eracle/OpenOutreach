...
1. **`handle_connect`** (`tasks/connect.py`, per-campaign) — Unified handler for both regular and partner campaigns via `ConnectStrategy` dataclass. `strategy_for(campaign, ...)` builds the right strategy based on `campaign.is_partner`: regular campaigns use `get_candidate()` from `pools.py` (composable generators: `ready_source` → `qualify_source` → `search_source`); partner campaigns use `get_partner_candidate()` from `pipeline/partner_pool.py` (queries `ProfileEmbedding` directly) with just-in-time Deal creation via `create_partner_deal()`. The handler itself has zero `is_partner` branches. Self-reschedules with `strategy.delay` (regular: ~10s, partner: `connect_delay_seconds / action_fraction`).
