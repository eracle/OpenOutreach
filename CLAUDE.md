...
The daemon (`daemon.py`) runs continuously with multi-campaign support using a **persistent task queue** backed by the `Task` Django model. Tasks are ordered by `scheduled_at` timestamp; the worker loop pops the oldest due task, executes it, and each task handler self-schedules follow-on tasks. On startup, `heal_tasks()` reconciles the queue with CRM state (recovers stale running tasks, seeds missing tasks). Daily/weekly rate limiters independently cap totals (shared across campaigns). Partner campaigns use probabilistic gating via `Campaign.action_fraction`. Regular campaigns are inversely gated.

Three task types (all plain handler functions in `linkedin/tasks/`, signature: `handle_*(task, session, qualifiers, partner_qualifier, kit_model)`):

1. **`handle_connect`** (per-campaign) — Pull-based architecture. Calls `get_candidate()` from `pools.py` which uses composable generators: `ready_source` → `qualify_source` → `search_source`. Each level tries its own pool, then pulls ONE item from upstream if empty. If a candidate is found, connects → PENDING, enqueues `check_pending` task for that profile. Self-reschedules with short delay (~10s) to burn daily connection quota. On rate limit or no candidates, reschedules with longer delay. Partner campaigns use `threshold=0` so all QUALIFIED profiles are promoted to READY_TO_CONNECT unconditionally.
2. **`handle_check_pending`** (per-profile) — checks one PENDING profile for acceptance → CONNECTED. Uses exponential backoff with multiplicative jitter: `delay = backoff * uniform(1.0, 1.0 + jitter_factor)`. Doubles `backoff_hours` each time a profile is still pending. On acceptance, enqueues `follow_up` task.
3. **`handle_follow_up`** (per-profile) — sends follow-up message to one CONNECTED profile → COMPLETED. On rate limit, reschedules with 1h delay.
