...
- **Discovery + enrichment**: `openoutreach/discovery.py` (Lead Finder `search`/`embed_row`, free) uses the blocking `submit_and_poll` transport; `emails/bettercontact.py` (paid finder) splits it into `submit(query) → request_id` + `poll_once(request_id) → PollOutcome` so the daemon never blocks on a poll (the `collect_email` leg owns the backoff). `core/pipeline/`: `icp.py` (LLM → Lead Finder filters, seeds the frontier's root node), `frontier.py` (**discovery as best-first graph search** over `DiscoveryQuery` nodes — replaces the single `icp_filters`/`discovery_offset` cursor; each move re-ranks fetched nodes by how many of their leads the GP would accept, picks the next node breadth-first before the qualifier's explore→exploit switch and by score after, and expands via `mutate.py`'s LLM-proposed queries), `mutate.py` (swappable LLM query-mutation generator), `discover.py` (fetch one picked node's page into first-touch `Lead`s), `qualify.py` → `ready_pool.py` (GP gate; its `count_accepted` is the frontier's per-node score) → `pools.py` (the composable generator chain; **interleaves discovery with qualification** — in exploit mode it pages in fresh leads whenever the pool holds nothing above the adaptive `min_positive_pool_prob` threshold, rather than qualifying the whole page first).
