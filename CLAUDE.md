...
1. **Connect** (scheduled, lazy chain) — Pull-based architecture. `execute()` calls `_get_candidate()` which checks the qualified pool filtered by `min_connect_prob` (default 0.9). If no candidate above threshold, calls `embed_one()`/`qualify_one()` from `lanes/qualify.py` and `search_one()` from `lanes/search.py` to backfill. After backfilling, re-checks the pool. If a candidate is found, connects → PENDING. Partner campaigns skip qualification and search.
...
Cold start (< 2 labels or single class) returns `None` from `predict`/`bald_scores`, and the connect lane defers to the LLM. As labels accumulate, the GPR progressively auto-decides more profiles, reducing LLM calls.
...
- **`pipeline/`** — Candidate sourcing, qualification, and pool management:
  - `qualify.py` — Embed and qualify orchestration. `embed_one(session, qualifier)` finds next un-embedded lead, lazily enriches via Voyager API, then embeds via fastembed. `qualify_one(session, qualifier)` selects a candidate via BALD/exploit strategy, attempts auto-decision via GP model, falls back to LLM. Private helpers: `_record_decision()` (updates `ProfileEmbedding` label, promotes or disqualifies lead), `_get_profile_text()` (enriches + builds text).
  ...
  - `pools.py` — Pool management and backfill orchestration. `top_above_threshold(session, qualifier, min_prob, pipeline=None)` queries `get_qualified_profiles()`, ranks via `qualifier.rank_profiles()`, checks top candidate's GPR probability against `min_prob`. `get_candidate(session, qualifier, min_prob, pipeline=None, is_partner=False)` calls `top_above_threshold()`, if empty and not partner, runs backfill chain (`embed_one`/`qualify_one`/`search_one`), then re-checks pool.
...
- **`conf.py:CAMPAIGN_CONFIG`** — Hardcoded timing/ML defaults:
  ...
  - `min_connect_prob` (0.9) — GPR probability threshold for connecting; qualified profiles below this are skipped
