...
- **Discovery + enrichment**: `openoutreach/discovery.py` (Lead Finder `search`/`embed_row`, free) uses the blocking `submit_and_poll` transport; `emails/bettercontact.py` (paid finder) splits it into `submit(query) → request_id` + `poll_once(request_id) → PollOutcome` so the daemon never blocks on a poll (the `collect_email` leg owns the backoff). `core/pipeline/`: `icp.py` (LLM → Lead Finder filters, generates the seed on cold start), `frontier.py` (**discovery as a lazy best-first walk** over `DiscoveryQuery` nodes — replaces the single `icp_filters`/`discovery_offset` cursor; only *fetched* nodes are persisted and the next query is computed lazily: `rerank` scores every non-exhausted node by how many of its leads the GP would accept, then `next_query` **bootstraps** (pages the seed linearly pre-exploit) / **deepens** (mines the best positive node in exploit) / **walls** (all-zero → one LLM query via `mutate.py`); an empty page marks that `params` `exhausted`), `mutate.py` (swappable LLM query-mutation generator; typed `_Filters` output — an unknown filter *key* returns an *empty page, not an error*, which the frontier would misread as end-of-depth, so the field names are enforced in the schema. Only `lead_seniority` is a closed vocabulary (`discovery.Seniority`, a `Literal`); every other family — including `lead_department`/`lead_function`, whose published "enum" matches nothing — is free text against a search index, where a miss just returns an empty page and costs one move), `discover.py` (fetch the selected query's page into first-touch `Lead`s), `qualify.py` → `ready_pool.py` (GP gate; its `count_accepted` is the frontier's per-node score) → `pools.py` (the composable generator chain; **interleaves discovery with qualification** — in exploit mode it pages in fresh leads whenever the pool holds nothing above the adaptive `min_positive_pool_prob` threshold, rather than qualifying the whole page first).
