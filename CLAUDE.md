...
- **`daemon.py`** — Main daemon loop. `LaneSchedule` class tracks `next_run` per major lane; `reschedule()` adds ±20% jitter. `_PromoRotator` logs rotating promotional messages every N ticks. `_rebuild_analytics()` runs `dbt run` with 120s timeout. `run_daemon(session)` creates `BayesianQualifier` (GPR pipeline, warm-started from historical labels), `LaneSchedule` objects per campaign for three major lanes, qualify lane for gap-filling, and a search lane as lowest-priority gap-filler. Rate limiting is handled by `LinkedInProfile` methods (DB-backed via `ActionLog`). Supports multi-campaign architecture: iterates `session.campaigns`, creates separate schedules per campaign, with shared qualifier. Partner campaigns use probabilistic gating (`action_fraction`); regular campaigns inversely gated with probability = `max(partner.action_fraction)`. Initializes embeddings table at startup. Also imports partner campaigns via `ml/hub.py`.
