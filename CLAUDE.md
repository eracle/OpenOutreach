...
3. **READY_TO_CONNECT gate** — After LLM qualifies a profile to QUALIFIED, it must also pass a GP confidence gate before becoming connectable. `promote_to_ready()` computes P(f > 0.5) for all QUALIFIED profiles, then applies temperature-scaled calibration (T = 1 + c/√n_obs, where c = `calibration_strength`, default 1.0) to push probabilities toward 0.5 when labels are sparse; those above `min_ready_to_connect_prob` (default 0.99) are promoted to READY_TO_CONNECT. As labels accumulate T → 1 and raw GP probabilities pass through. During cold start (GP not fitted), no profiles reach READY_TO_CONNECT — the connect lane keeps triggering qualifications until the model can fit.
...
- **`ml/qualifier.py:BayesianQualifier`** — Pipeline(PCA, StandardScaler, GaussianProcessRegressor) with lazy refit and temperature-scaled calibration (`_calibrate_probs`: T = 1 + c/√n_obs pushes probabilities toward 0.5 with sparse data, converges to raw GP as labels grow). PCA dimensions selected via LML cross-validation. `update(embedding, label)` appends to training data and invalidates fit. `predict(embedding)` returns `(prob, entropy, posterior_std)` 3-tuple or `None` if unfitted; prob is calibrated, std is raw. `predicted_probs(embeddings)` returns calibrated probability array. `bald_scores(embeddings)` computes BALD via MC sampling from GP posterior (raw — exploration unaffected). `rank_profiles(profiles, session, pipeline=None)` sorts by calibrated probability (descending); uses `load_embedding()` from `crm_profiles` for lazy embedding loading. `explain(profile, session)` returns human-readable explanation. `warm_start(X, y)` bulk-loads historical labels and fits once. Fitted pipeline persisted via `joblib` to per-campaign path (`model_path_for_campaign()`). Also exports `qualify_with_llm(profile_text, product_docs, campaign_objective)` for LLM-based lead qualification with structured output (`QualificationDecision`).
...
- **`conf.py:CAMPAIGN_CONFIG`** — Hardcoded timing/ML defaults:
  ...
  - `calibration_strength` (1.0) — temperature scaling strength for GP probabilities: T = 1 + c/√n_obs; prevents overconfident promotion with sparse training data
