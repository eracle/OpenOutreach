...
5. **Qualify** (gap-filling) — two-phase: (1) embeds ENRICHED profiles that lack embeddings, (2) qualifies embedded profiles using active learning (classifier auto-decision for high/low confidence, LLM for uncertain profiles) → QUALIFIED or DISQUALIFIED. Triggers classifier retraining when enough new labels accumulate.
...
- **`lanes/`** — Action lanes executed by the daemon:
  ...
  - `qualify.py` — Two-phase qualification lane: (1) embeds ENRICHED profiles that lack embeddings (backfill), (2) qualifies embedded profiles via active learning — classifier auto-decides for high/low confidence, LLM (`qualify_lead.j2` prompt) for uncertain profiles → QUALIFIED or DISQUALIFIED. Triggers classifier retraining when enough new labels accumulate.
  - `connect.py` — ML-ranks QUALIFIED profiles, sends connection requests. Catches pre-existing connections missed by enrich (when `connection_degree` was None at scrape time).
...
- **`ml/embeddings.py`** — DuckDB vector store for profile embeddings. Uses `fastembed` (BAAI/bge-small-en-v1.5 by default) for 384-dim embeddings. HNSW index for cosine similarity search. Functions: `embed_text()`, `embed_profile()`, `store_embedding()`, `store_label()`, `get_positive_centroid()`, `get_unlabeled_profiles()`, `get_labeled_data()`, `count_labeled()`, `get_embedded_lead_ids()`, `ensure_embeddings_table()`.
- **`ml/qualifier.py:QualificationScorer`** — BaggingClassifier(HistGradientBoostingClassifier) + Thompson Sampling for profile ranking. Trains on 384-dim embeddings from the `profile_embeddings` DuckDB table. Scoring chain: (1) seeds ranked first, (2) trained classifier → sample random estimator (Thompson Sampling), (3) no classifier but seeds exist → cosine similarity to positive centroid + Thompson Sampling, (4) FIFO fallback. Also exports `qualify_profile_llm()` for LLM-based lead qualification with structured output.
...
- **`seeds.py`** — Seed URL loader. `load_seed_urls()` reads from `SEED_URLS_FILE` (CSV with `url` header). `ensure_seeds()` creates DISCOVERED leads tagged with `{"seed": true}` in `deal.next_step`. Seeds are always ranked first by the scorer and auto-labeled as positive in the embeddings table.
...
- **`conf.py`** — Loads config from `.env` and `assets/accounts.secrets.yaml`. Exports `CAMPAIGN_CONFIG` dict (rate limits, timing, qualification thresholds, embedding model), `PRODUCT_DOCS_FILE` / `CAMPAIGN_OBJECTIVE_FILE` / `EMBEDDINGS_DB` / `SEED_URLS_FILE` paths. `LLM_API_KEY` is required (raises `ValueError` if missing). All paths derived from `ASSETS_DIR`.
...
- **`campaign:` section** — `connect.daily_limit`, `connect.weekly_limit`, `check_pending.recheck_after_hours` (base interval, doubles per profile via exponential backoff), `follow_up.daily_limit`, `follow_up.existing_connections` (false = ignore pre-existing connections), `enrich_min_interval` (floor seconds between enrichment API calls, default 1), `working_hours.start` / `working_hours.end` (HH:MM, default 09:00–18:00, OS local timezone), `input_csv` (seed URLs CSV path relative to assets/, default `inputs/urls.csv`).
- **`campaign.qualification:` section** — `high_threshold` (default 0.80), `low_threshold` (default 0.20), `uncertainty_threshold` (default 0.15), `min_training_samples` (default 40), `min_class_ratio` (default 0.30), `retrain_every` (default 200), `n_estimators` (default 30), `embedding_model` (default `BAAI/bge-small-en-v1.5`).
- **`assets/inputs/urls.csv`** — Seed URLs CSV with `url` header. Required at startup (raises `FileNotFoundError` if missing). Seed profiles are loaded as DISCOVERED leads and auto-labeled as positive.
...
ML/Embeddings: `scikit-learn`, `duckdb`, `fastembed`
