...
`LinkedInProfile`/`SearchKeyword`/legacy channel `ActionLog` models were deleted in `legacy/0012`).
...
- `oo` — JSON operator control plane for agents and scripts. Read-only commands expose daemon state (`status`, `campaign list`, `lead list`, `task list`); `email send-next` dry-runs or executes one opener send with an explicit idempotency key; `audit list` reads the `ActionLog`. Every JSON path emits the same envelope (`ok`, `command`, `status`, `dry_run`, `action_id`, `result`, `error`, `warnings`) so an external orchestrator can handle success, expected errors, and duplicates without scraping logs.
...
QUALIFIED ─(GP rank gate)─▶ READY_TO_FIND_EMAIL ─(find_email/submit)─▶ FINDING_EMAIL ─(collect_email/poll)─▶ hit:  READY_TO_EMAIL
 ...
 (no email yet)              paid lookup               request_id in task payload              │
                          ...
                          READY_TO_EMAIL ──(claim)──▶ SENDING_EMAIL ──(email opener)──▶ EMAILED ⟲ (agentic follow-up) ──▶ COMPLETED / FAILED
...
- **`SENDING_EMAIL`** — a transient claim state set before compose/SMTP work starts. It prevents a concurrent CLI/daemon send from selecting the same deal while the external send is in flight. If the send fails before SMTP accepts it, the claim is released back to `READY_TO_EMAIL`; once a Message-ID exists, the send is treated as committed and failures are surfaced through the action/task failure path.
...
1. **`flush_find_email_queue`** — mints one submit slot when there's mailbox send-headroom for the result *today*: `Mailbox.objects.remaining_today()` minus everything already in the send pipeline (`READY_TO_EMAIL + SENDING_EMAIL + FINDING_EMAIL`). One slot per call (the handler is the pipeline *pump*, so a batch would fan out discovery). No-op unless a mailbox is connected **and** the finder is configured. The GP gate rations *which* leads qualify; the send cap bounds *how many* lookups ride the pipeline — so we never resolve an email we couldn't send today, and free misses re-open the gate at no cost.
...
3. **`handle_email`** (`tasks/send.py`) — calls `send_next_email(session)`, which picks the least-loaded under-cap `Mailbox`, atomically claims the oldest `READY_TO_EMAIL` deal by moving it to `SENDING_EMAIL`, materializes the profile summary, composes the opener (`core/agents/email_opener.py`), sends over SMTP (`emails/sender.py`, BCC = the operator's own address), then `_record_sent_email` writes the email fields, the outgoing opener `ChatMessage`, and `state=EMAILED`. A claim lost to another worker is skipped; a pre-SMTP failure releases the claim back to `READY_TO_EMAIL`. `next_follow_up_at` is seeded from the opener agent's own `follow_up_hours`.
...
## Operator JSON Control Plane

`core/management/commands/oo.py` is the machine-facing interface for external operators such as Hermes. It does not replace the daemon; it exposes deterministic controls around the same models and handlers.
...
.venv/bin/python manage.py oo email send-next --campaign "Campaign" --confirm --idempotency-key key-2 --json
.venv/bin/python manage.py oo email send-next --campaign "Campaign" --non-interactive --idempotency-key key-3 --json
.venv/bin/python manage.py oo audit list --json --limit 50
```

`email send-next` always requires exactly one execution mode and a non-blank idempotency key. `--dry-run` creates a planned `ActionLog` row without composing or sending. `--confirm` and `--non-interactive` execute the same `send_next_email(session)` helper the daemon uses. Duplicate keys return duplicate metadata and never send twice; conflicting keys are rejected. Expected no-work cases (`no_eligible_email`), invalid input, missing campaigns, and send failures are returned as JSON envelopes rather than tracebacks.

`audit list` reads the `ActionLog` newest first. Limits default to 50 and are capped at 200 so agents can poll safely.
...
- **`core`** — Engine: `SiteConfig`, `Campaign`, `Task`, `ActionLog` models; daemon, scheduler, LLM factory, onboarding, the ML/discovery/qualify pipeline, the two agents, session, geo, JSON management control plane, vendored mem0.
...
- **ActionLog** (`core/models.py`) — audit/idempotency row for explicit operator actions. Fields: `action_type`, `target_type`, `target_id`, `payload_hash`, `idempotency_key`, `status` (`planned`, `running`, `succeeded`, `failed`, `duplicate`), `result`, `error_type`, `error_message`, timestamps. A partial unique constraint on `(action_type, idempotency_key)` when the key is non-blank makes scripted actions safe to retry. `core/actions.py:run_logged_action` owns the dry-run/execute/duplicate/conflict semantics.
...
- **`core/scheduler.py`** — the only creator of `Task` rows: `flush_find_email_queue` (send-headroom-gated submit drain; counts `READY_TO_EMAIL`, claimed `SENDING_EMAIL`, and `FINDING_EMAIL` against today's send headroom), `flush_email_queue` / `flush_follow_up_queue` (eager drains), `schedule_collect_email` (the bound, self-chaining poll), and `reconcile`. No Poisson pacing or spend cap.
- **`core/actions.py`** — idempotent operator-action wrapper around `ActionLog`. It hashes payloads, rejects conflicting reused keys, records dry-run plans, atomically claims planned rows for real execution, persists success/failure result data, and returns duplicate metadata without re-executing.
...
- **`discovery.py`** — Lead Finder client: `search(filters, limit, offset)` (ICP search → lead rows, free), `profile_text_for(row)` + `embed_row(row)` (the qualifier's text/vector — the base fields keep the pre-pivot field order so old and new vectors stay comparable, with `EXTRA_TEXT_FIELDS`/`EXTRA_TEXT_LIST_FIELDS` appended only when present to enrich sparse rows). Shares `submit_and_poll` with `emails/bettercontact.py`.
