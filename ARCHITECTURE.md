...
1. **Discovery** feeds the pool as a **lazy best-first walk** (`core/pipeline/frontier.py`, replacing the old single `icp_filters`/`discovery_offset` cursor): each campaign owns many `DiscoveryQuery` nodes — a **set of `Clause` rows plus an `offset`**, **fetched once** into first-touch `Lead`s (`Lead.discovered_by`). A `Clause` is one `(family, value)` pair (`lead_job_title = Founder`), globally unique and shared across campaigns; a node ANDs **at most one clause per family**, and `discovery.filters_for` is the only place that becomes provider JSON — it raises on a second clause of the same family. There is no `params` blob and no include-list: an include-list is an OR, and an OR is strictly dominated, because a filter doesn't narrow a result set — it *moves a ~10k-row sampling window* over a corpus ordered by provider preference, so five titles ORed share one window where five queries would get five, for free. Dedup is `(campaign, clause_key, offset)`, `clause_key` being the sha256 of the sorted clause set (a column because no unique constraint can span an M2M). Only fetched nodes are persisted; the next query is computed lazily, not queued. `discover.py:discover(session)` is one move: `next_query` selects a query and its page is fetched. A node's value is **measured, not predicted** — `frontier.node_stats` is one `GROUP BY` returning `(examined, qualified)` over the node's first-touch leads' deals, computed per move and **never stored** (there is no `score` column). Two moves fall out of one count, interleaved **1:1**: **deepen** (a conjunction has `qualified > 0` summed over *all its offsets* — `frontier.line_stats`, grouped by `clause_key` — and its line is still open: page it one offset deeper) or **visit** (open the next conjunction from `descend.py`, and let `mutate.py` ask the LLM for fresh clauses only once the pool spans nothing new). Deepen is **per conjunction, summed over offsets**: a lead qualified at offset 0 keeps paging the line deeper and does not expire when one deep page qualifies nobody — the reversal of an earlier **per-node** rule. That rule existed to stop a non-terminating walk (paging from a line's `Max(offset)` mined a `headcount 1–?` singleton forever, since a broad singleton never empties); it is safe to drop now because `descend.py` is **deepest-only** — the only lines that exist are deep conjunctions, which empty in bounded pages — and the 1:1 interleave caps any one line at half the moves. The 1:1 balance is stateless strict alternation: a deepen leaves a page at `offset > 0`, a visit a conjunction at `offset == 0`, so the newest node's offset says which came last and `next_query` takes the other (deepen is unavailable until something qualifies, so cold-start moves are all visits — no catch-up burst). The frontier only ever asks whether `qualified` is *positive* — it never ranks barren conjunctions against each other. The **bootstrap** and **wall** regimes are gone (the seed is simply the deepest conjunction the pool spans), but the `examined == 0` is-*unknown*-not-barren distinction is **load-bearing**: a just-fetched frontier page has `examined == 0`, so it does not vote and cold-start moves fall to `visit` until qualification rules on it — deepen is deferred to the move after the labels land, not lost. Exhaustion is reactive and is the **only** thing that retires a line: an empty page marks that clause-set line `exhausted`, whereas a barren yield never prunes. **The frontier reads no signal from the GP** — see "Three signals, three jobs" below. The per-node clause set / offset and `(leads, examined, qualified)` are inspectable in Django Admin, as is the `Clause` vocabulary itself.

   **The visit composes before it invents, and it is deepest-only.** There is no cheap liveness probe: a query execution already answers the only question a probe asked ("does this conjunction match anybody?") and pays for the rows besides, so `descend.py` is a pure lookup that makes **no provider call at all** — it hands the frontier the next unfetched conjunction and `discover.py` finds out by fetching it. It returns the **maximal** conjunctions — one value from *every* family, the deepest queries the pool spans — and nothing shallower, because a shorter conjunction is a strictly broader window over the same corpus (it returns a superset of the deeper query's people, drifting off the niche toward the provider's famous-company head). Volume from a query that works comes from paging it deeper (deepen), not from dropping its clauses. A conjunction is widened for exactly one reason: **its deeper parent matched nobody.** An empty query unlocks its drop-one-clause children (`{a,b,c}` empty → `{a,b}`, `{a,c}`, `{b,c}`), recursively, until queries start matching people — anti-monotonicity run forward, since an empty query's supersets are empty but its subsets may not be. An `EmptyClauseSet` therefore does double duty: it prunes every superset of an empty conjunction (the subset test, `_is_pruned`) **and** unlocks that conjunction's children. **Only emptiness widens; yield never does** — a barren node says *those people aren't our ICP*, so widening it would only add more of the wrong people, and an empty conjunction convicts the whole set and never a clause inside it (`lead_department: Sales` returns rows alone yet sat in six 0-row conjunctions). When the pool spans no unfetched, unpruned conjunction, `descend` returns `[]` and the LLM refill adds clauses that recombine into **new maximal conjunctions**, so the deepest layer grows rather than the walk drifting shallow.
