...
1. **Discovery** feeds the pool as a **best-first graph search** (`core/pipeline/frontier.py`, replacing the old single `icp_filters`/`discovery_offset` cursor): each campaign owns many `DiscoveryQuery` nodes — a `(Lead Finder filter dict, offset)` fetched once into first-touch `Lead`s (`Lead.discovered_by`). `discover.py:discover(session, qualifier)` is one move: re-rank the fetched nodes against the current GP (score = `ready_pool.count_accepted`, how many of the node's leads clear the acceptance gate), pick the next PENDING node (breadth-first before the qualifier's explore→exploit switch, by inherited parent score after), fetch its page, then `expand` — deepen the node and, when breadth thins, enqueue LLM-proposed mutations (`mutate.py`). The seed comes from `icp.py` (`generate_icp_spec`). The qualify chain calls `discover` both when its candidate pool goes dry **and** — in exploit mode — when the pool holds no candidate above the adaptive `min_positive_pool_prob` threshold, so discovery stays interleaved with qualification instead of draining the whole page first. The per-node params/offset/score/yield are inspectable in Django Admin.
...
- **Campaign** (`core/models.py`) — `name` (unique), `users` (M2M to `User`), `product_docs`, `campaign_target`, `booking_link`, `is_freemium`, `action_fraction`, `seed_public_ids`, `model_blob` (per-campaign GP), `country_code` (the ICP's target country, stamped on discovered leads for the geo-gate; set by `frontier.ensure_seed` from the LLM spec). Discovery state lives in `DiscoveryQuery` nodes, not on the campaign (the old `icp_filters`/`discovery_offset` cursor was seeded into a node and dropped in migration 0007).
- **DiscoveryQuery** (`core/models.py`) — one node in a campaign's discovery graph: `campaign` FK, `params` (Lead Finder filter dict) + `params_hash` (sha256 of canonicalized params, the dedup key) + `offset`, `status` (`pending` fetchable → `fetched` scored → `retired`), `score` (count of the node's leads the GP would accept; null pre-exploit), `parent` self-FK (the node it deepened/mutated from). Unique on `(campaign, params_hash, offset)`. `lead_embeddings` gathers its first-touch leads' embeddings on demand for the re-rank.
