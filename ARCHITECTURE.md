...
1. **Discovery** feeds the pool: `core/pipeline/discover.py:discover` pages the campaign ICP (`core/pipeline/icp.py`, cached on `Campaign.icp_filters`) from Lead Finder into embedded `Lead`s; the qualify chain calls it both when its candidate pool goes dry **and** — in exploit mode — when the pool holds no candidate above the adaptive `min_positive_pool_prob` threshold, so discovery stays interleaved with qualification instead of draining the whole page first.
