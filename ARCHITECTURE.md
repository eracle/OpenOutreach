...
- **`conf.py:CAMPAIGN_CONFIG`** — `min_gp_confidence` (0.9, the GP rank gate), `positive_pool_percentile` (25 — the discovery-interleave gate, read as a percentile of the GP's scores for its *proven positives* via `BayesianQualifier.positive_score_floor`, **not** an absolute probability: `predict_probs` is P(latent f > 0.5), not a calibrated rate, so a fixed cutoff means a different thing in every campaign and goes stale as the model trains — a campaign converting 4% can still score its positives ~0.78), `qualification_n_mc_samples` (100), `embedding_model` (`BAAI/bge-small-en-v1.5`), and the human-rhythm knobs `burst_min/max_seconds` (45–65 min), `break_min/max_seconds` (10–20 min).
