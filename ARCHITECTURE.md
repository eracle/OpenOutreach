...
mailbox         field-by-field SMTP box + signature → auth-check → Mailbox row; retries with values retained
...
- The **signature** is asked per mailbox (last field of the `mailbox` step, optional) and appended to every send from that box by `sender._sign` — openers and follow-ups alike, since both go through `send_email`. It lives on `Mailbox` rather than `SiteConfig` because it is part of the sending identity, and it stays editable in the Django Admin. The email prompts still forbid the agent from signing its own drafts (`prompts/_outreach_base.j2`), so there is exactly one sign-off.
...
1. **Discovery** feeds the pool as a **lazy best-first walk** (`core/pipeline/frontier.py`, replacing the old single `icp_filters`/`discovery_offset` cursor): each campaign owns many `DiscoveryQuery` nodes — a `(Lead Finder filter dict, offset)` **fetched once** into first-touch `Lead`s (`Lead.discovered_by`). Only fetched nodes are persisted; the next query is computed lazily, not queued. `discover.py:discover(session, qualifier)` is one move: `rerank` the fetched nodes against the current GP (score = `ready_pool.count_accepted`, how many of the node's leads clear the acceptance gate), then `next_query` selects one — **bootstrap** (pre-exploit: page the seed linearly, cold-starting it from `icp.py:generate_icp_spec` when no node exists yet), **deepen** (exploit, best positive node: fetch its line's next page), or **wall** (exploit, all scores 0: one LLM-proposed query via `mutate.py`). Exhaustion is reactive: an empty page marks that `params` line `exhausted` so it's never re-picked. The qualify chain calls `discover` both when its candidate pool goes dry **and** — in exploit mode — when the pool holds no candidate above the adaptive `min_positive_pool_prob` threshold, so discovery stays interleaved with qualification instead of draining the whole page first. The per-node params/offset/score/yield are inspectable in Django Admin.
...
- **Campaign** (`core/models.py`) — `name` (unique), `users` (M2M to `User`), `product_docs`, `campaign_target`, `booking_link`, `is_freemium`, `action_fraction`, `seed_public_ids`, `model_blob` (per-campaign GP), `country_code` (the ICP's target country, stamped on discovered leads for the geo-gate; set by `frontier.generate_seed` from the LLM spec on cold start). Discovery state lives in `DiscoveryQuery` nodes, not on the campaign (the old `icp_filters`/`discovery_offset` cursor was dropped in migration 0007; the seed is regenerated on cold start, then embodied by its fetched nodes).
- **DiscoveryQuery** (`core/models.py`) — one **fetched** node in a campaign's discovery walk: `campaign` FK, `params` (Lead Finder filter dict) + `params_hash` (sha256 of canonicalized params, the dedup key) + `offset`, `exhausted` (bool — set on every node of a `params` line whose deepen hit an empty page; excluded from selection), `score` (count of the node's leads the GP would accept; null pre-exploit). Unique on `(campaign, params_hash, offset)`. Only fetched nodes exist — no pending queue, no `parent` provenance. `lead_embeddings` gathers its first-touch leads' embeddings on demand for the re-rank.
...
- **Mailbox** (`emails/models.py`) — one SMTP inbox: `host`/`port` (default `smtp.gmail.com:587`), `imap_host`/`imap_port` (default `imap.gmail.com:993` — the read side for the reply loop, same app password), `username`, `password`, `from_address`, `signature` (the sign-off appended to every send from this box — per box because it is part of the sending identity; blank = none), `daily_limit` (warm-safe sends/day, default `DEFAULT_EMAIL_DAILY_LIMIT`). A row exists only once its credentials pass the import auth-check (no health API). Manager: `remaining_today()` (Σ per-box headroom), `least_loaded_under_cap()`; instance `sent_today()` (outgoing ChatMessages on this box's deals since local midnight), `headroom_today()`. `has_mailbox()` is the "email is a viable channel" gate.
...
- **`core/pipeline/`** — `icp.py` (`generate_icp_spec`: one LLM pass → Lead Finder filters, seeds the frontier root), `frontier.py` (lazy best-first walk over `DiscoveryQuery` nodes: `generate_seed`, `rerank`, `next_query` (bootstrap/deepen/wall), `persist_fetched`, `mark_exhausted`, `params_hash`/dedup), `mutate.py` (`generate_mutation` behind a swappable `set_generator` interface: LLM proposes one new filter set from past-query stats, as a **typed** `_Filters` output — Lead Finder returns an empty page rather than an error for an unknown *key*, and the frontier would read that as end-of-depth, so the field names are constrained in the schema. Values are not: only `lead_seniority` is a real closed vocabulary (a `Literal`); the rest — including `lead_department`/`lead_function`, documented as enums but actually free text — are search terms, where a miss is a normal empty page), `discover.py` (`discover(session, qualifier)`: one move — fetch the picked node into first-touch Leads), `qualify.py` (`run_qualification` / `fetch_qualification_candidates` — reads `Lead.profile_text`, no scrape), `ready_pool.py` (GP gate: `promote_to_ready`, `find_ready_candidate`; `acceptance_threshold`/`count_accepted` shared with the frontier), `pools.py` (composable generators `qualify_source → ready_source → find_candidate`; `_needs_more_discovery` interleaves discovery with qualification — pages in fresh leads when the pool is dry or, in exploit mode, holds nothing above the adaptive threshold), `freemium_pool.py` (`find_freemium_candidate`).
...
- **`conf.py:CAMPAIGN_CONFIG`** — `min_gp_confidence` (0.9, the GP rank gate), `min_positive_pool_prob` (0.20, base of the adaptive discovery-interleave threshold `max(0, base - 1/sqrt(n_obs))`), `qualification_n_mc_samples` (100), `embedding_model` (`BAAI/bge-small-en-v1.5`), and the human-rhythm knobs `burst_min/max_seconds` (45–65 min), `break_min/max_seconds` (10–20 min).
