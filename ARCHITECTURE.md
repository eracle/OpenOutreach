...
- **Lead** (`crm/models/lead.py`) — Keyed on `profile_url` (unique URLField) — the discovery provider's per-person URL, the opaque identity/lookup key, **stored but never fetched** (re-fetching would reopen the LinkedIn surface the pivot removes). `country_code` = ISO-3166 alpha-2, stamped from the discovery ICP's target country (drives the contacts-store geo-gate; blank = unknown → never contributed). `embedding` = 384-dim float32 BinaryField (nullable), built at discovery. `profile_text` = TextField — the firmographic text (headline/location/industry/title/company/company-description) built from the Lead Finder row at discovery, the LLM qualifier's input; no LinkedIn re-scrape. `email` = nullable EmailField — the enrichment-API (BetterContact) result; null = not found. `disqualified` = permanent exclusion. `resolve_email()` populates `email` via the finder (tri-state: True hit / False genuine-miss / None finder-unavailable; cached on a hit) — called from the **find-email leg** (`emails/tasks/find_email.py`), not inline at qualification. On a hit that leg routes the Deal READY_TO_FIND_EMAIL → `DealState.READY_TO_EMAIL`. `to_profile_dict()` → `{lead_id, profile_url, meta}`. `embedding_array` property for numpy access. `get_labeled_arrays(campaign)` classmethod returns (X, y) for GP warm start (non-FAILED → 1, FAILED+wrong_fit → 0, other FAILED → skipped). Leads are created **browserless** from Lead Finder rows via `core/db/leads.py:create_lead(row, country_code)` (freemium seeds via `core/setup/freemium.py`); there are no LinkedIn scrape accessors (`get_profile`/`get_urn`/`capture_contact_info`/`embed_from_profile` are gone with the channel).
...
- **`emails/tasks/find_email.py`** — `handle_find_email`, the paid-lookup leg: routes a rank-gated deal READY_TO_FIND_EMAIL → READY_TO_EMAIL on a finder hit, → FAILED on a miss.
- **`emails/tasks/send.py`** — `handle_email`, the single Layer-1 opener (→ EMAILED).
- **`emails/tasks/follow_up.py`** — `handle_follow_up`, drains due EMAILED deals.
- **`discovery.py`** — Lead Finder client: `search(filters, limit, offset)`, `embed_row(row)`, `profile_text_for(row)` (shared `submit_and_poll` transport with `emails/bettercontact.py`).
- **`core/pipeline/discover.py`** — `discover(session)`: pages the campaign ICP from Lead Finder into embedded Leads (`create_lead`); the top of the funnel, upstream of qualify.
- **`core/pipeline/icp.py`** — `icp_for()` / `generate_icp_spec()`: single LLM pass mapping `product_docs + objective` → Lead Finder filters, cached on `Campaign.icp_filters`.
- **`core/pipeline/qualify.py`** — `run_qualification()`, `fetch_qualification_candidates()` (reads `Lead.profile_text` for the LLM; no scrape).
- **`core/pipeline/ready_pool.py`** — GP confidence gate, `promote_to_ready()` (QUALIFIED → READY_TO_FIND_EMAIL).
- **`core/pipeline/pools.py`** — Composable generators: `qualify_source` (discovers a fresh ICP page when the pool goes dry) → `ready_source` → `find_candidate`.
- **`core/pipeline/freemium_pool.py`** — Seed priority + undiscovered pool, ranked by qualifier.
- **`core/ml/qualifier.py`** — `Qualifier` protocol, `BayesianQualifier`, `KitQualifier`, `qualify_with_llm()`.
- **`core/ml/embeddings.py`** — FastEmbed utilities, `embed_text()`, `embed_texts()`.
- **`core/ml/hub.py`** — HuggingFace kit loader (`fetch_kit()`).
