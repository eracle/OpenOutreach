...
- **`core/pipeline/`** — `icp.py` (`generate_seed`: the cold start — one LLM pass → the seed conjunction, **one value per family** (scalar `ICPSpec`), folded onto `Campaign.clauses`; the seed is the single most-precise starting query and the whole starting pool — one maximal, and minting supplies the alternatives), `select.py` (**the selector**: `next_query(campaign, qualifier)` prefilters every fetchable maximal by a cheap composed score on the live axis (`_prefilter`, `PREFILTER_K` — small K for exploit, larger for explore), exact-embeds only the top-K, scores them via `qualifier.acquisition_scores` and returns the argmax — fresh (offset 0) or deepen (next page); cold start falls back to seed-first, `None` = saturated. Owns `persist_fetched`/`mark_exhausted`/`record_empty`/`clause_key`), `mint.py` (`mint_clauses`: grow the pool from the qualified leads' real attributes; throughput OR saturation triggers, best-effort on LLM outage), `discover.py` (`discover(session, qualifier)`: seed → throughput mint → fetch the GP's pick into first-touch `Lead`s with keyword-injected embeddings → record the two shapes of empty → saturation mint), `qualify.py` (`run_qualification` / `fetch_qualification_candidates` — reads `Lead.profile_text`, no scrape), `ready_pool.py` (GP gate: `promote_to_ready`, `find_ready_candidate`; `min_gp_confidence` is the spend gate **and nothing else**), `pools.py` (composable generators `qualify_source → ready_source → find_candidate`; `consumable_candidates` decides **consume** vs **cold start** — see its module docstring), `freemium_pool.py` (`find_freemium_candidate`).
