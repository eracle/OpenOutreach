...
1. **Discovery** feeds the pool as a **lazy best-first walk** (`core/pipeline/frontier.py`, replacing the old single `icp_filters`/`discovery_offset` cursor): each campaign owns many `DiscoveryQuery` nodes — a `(Lead Finder filter dict, offset)` **fetched once** into first-touch `Lead`s (`Lead.discovered_by`). Only fetched nodes are persisted; the next query is computed lazily, not queued. `discover.py:discover(session, qualifier)` is one move: `rerank` the fetched nodes against the current GP (score = `ready_pool.count_accepted`, how many of the node's leads clear the acceptance gate), then `next_query` selects one — **bootstrap** (pre-exploit: page the seed linearly, cold-starting it from `icp.py:generate_icp_spec` when no node exists yet), **deepen** (exploit, best positive node: fetch its line's next page), or **wall** (exploit, all scores 0: one LLM-proposed query via `mutate.py`). Exhaustion is reactive: an empty page marks that `params` line `exhausted` so it's never re-picked. The qualify chain calls `discover` **only when its candidate pool goes dry** — the pool drains first, which is what feeds the frontier per-node measurements (a node nobody examined has no measured value). There is deliberately **no "is this pool promising?" gate**: two were tried and both were constants in disguise (see `pools.py`'s module docstring), because every such gate compares an **out-of-sample** candidate score against a bar drawn from **in-sample** ones, and a fitted GP never puts those two populations on the same scale. The per-node params/offset/score/yield are inspectable in Django Admin.
