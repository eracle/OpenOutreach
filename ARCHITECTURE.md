...
1. **Discovery** feeds the pool as a **lazy best-first walk** (`core/pipeline/frontier.py`, replacing the old single `icp_filters`/`discovery_offset` cursor): each campaign owns many `DiscoveryQuery` nodes — a **set of `Clause` rows plus an `offset`**, **fetched once** into first-touch `Lead`s (`Lead.discovered_by`). A `Clause` is one `(family, value)` pair (`lead_job_title = Founder`), globally unique and shared across campaigns; a node ANDs **at most one clause per family**, and `discovery.filters_for` is the only place that becomes provider JSON — it raises on a second clause of the same family. There is no `params` blob and no include-list: an include-list is an OR, and an OR is strictly dominated, because a filter doesn't narrow a result set — it *moves a ~10k-row sampling window* over a corpus ordered by provider preference, so five titles ORed share one window where five queries would get five, for free. Dedup is `(campaign, clause_key, offset)`, `clause_key` being the sha256 of the sorted clause set (a column because no unique constraint can span an M2M). Only fetched nodes are persisted; the next query is computed lazily, not queued. `discover.py:discover(session)` is one move: `next_query` selects a query and its page is fetched. A node's value is **measured, not predicted** — `frontier.node_stats` is one `GROUP BY` returning `(examined, qualified)` over the node's first-touch leads' deals, computed per move and **never stored** (there is no `score` column). The regime then falls out of those counts, with no threshold to tune: **bootstrap** (no node has an examined lead, so nothing is rankable: page the seed linearly, cold-starting it from `icp.py:generate_seed` when no node exists yet), **deepen** (the best rankable node has `qualified > 0`: fetch its line's next page), or **wall** (every rankable node has `qualified == 0`: one new query, **composed** from the campaign's clause pool by `descend.py` and only *invented* by `mutate.py` when the pool spans nothing untried). `examined == 0` means *unknown*, not barren, so an unsampled node is unrankable and can neither be deepened nor vote for a wall. Exhaustion is reactive and is the **only** thing that retires a line: an empty page marks that clause-set line `exhausted`, whereas a barren yield never prunes. **The frontier reads no signal from the GP** — see "Three signals, three jobs" below. The per-node clause set / offset and `(leads, examined, qualified)` are inspectable in Django Admin, as is the `Clause` vocabulary itself.

   **The wall composes before it invents — probe and fetch are different jobs.** A probe is `discovery.probe` (`limit=1`, creates no leads, answers "does this conjunction exist?"); a fetch is `limit=100` and creates up to 100 leads, which cost *examination* — the one genuinely scarce resource. `descend.py` spends probes and hands the frontier only a verified non-empty conjunction: it sweeps every unprobed pool clause **alone** (`Clause.is_live`), which is the highest-value call in the lattice because emptiness is anti-monotone — a clause the index carries nowhere kills every conjunction containing it, so `lead_location: Europe` dies on one probe instead of poisoning query after query. It then walks the conjunctions the surviving pool spans, deepest-first, skipping anything already fetched or pruned, and blacklists each empty conjunction it meets (`EmptyClauseSet`; a candidate is dead iff some recorded set is a **subset** of it). Probing the short conjunctions while fetching only deep ones is the point: a short conjunction matches millions and its window fills with the provider's famous-company head (`{lead_seniority: founder}` → Meta, Meta, Meta), so probing maps structure where fetching would pump the head into the pool. **Only emptiness prunes; yield never does** — a wall says *those people aren't our ICP*, which retires nothing, and an empty conjunction convicts the whole set and never a clause inside it (`lead_department: Sales` returns rows alone yet sat in six 0-row conjunctions). A clause is retired only by its own singleton probe, which is sound.

   The qualify chain calls `discover` **only when its candidate pool goes dry** — the pool drains first, which is what feeds the frontier its per-node counts (a node nobody examined has no measured value). There is deliberately **no "is this pool promising?" gate**: two were tried and both were constants in disguise (see `pools.py`'s module docstring), because every such gate compares an **out-of-sample** candidate score against a bar drawn from **in-sample** ones, and a fitted GP never puts those two populations on the same scale.

   **Three signals, three jobs — no crossing over.** The frontier steers on ground-truth node counts; the GP/BALD (`qualifier.acquisition_scores`) picks *which lead to qualify next*; `min_gp_confidence` (`ready_pool.acceptance_threshold`) is *only* the spend gate on the paid lookup. Discovery previously borrowed both of the others and was broken by each: it scored nodes by how many leads cleared the 0.9 **spend** bar (none ever did — that bar is calibrated on labelled leads the GP has memorized at 0.755–0.829, while everything unseen regresses toward the prior at 0.121–0.327, so the populations never overlap), and it read the qualifier's `n_neg > n_pos` **balance-driven acquisition strategy** as if it were a competence gate (at any realistic base rate negatives lead forever, so the walk never left bootstrap). Every node scored 0, the walk read a permanent wall, and `deepen` never fired once.
...
- **Clause** (`core/models.py`) — one `(family, value)` pair (`lead_job_title = Founder`). Globally unique on `(family, value)` and shared across campaigns: a clause is not campaign-specific, and giving it a campaign would duplicate a fact `DiscoveryQuery` already owns. `family` is constrained to `discovery.FILTER_FAMILIES` (the field names are the provider contract, and an unknown key is silently *dropped* — you get the unfiltered page, with rows, reading as success); `value` is deliberately unconstrained, because outside `lead_seniority` these are free-text search terms and a value the index lacks is just an empty page. `is_live` (nullable bool) is the singleton probe's verdict — NULL means *never probed*, and only a probe of the clause **on its own** may write it: a clause that returns nothing alone matches nothing in any conjunction (anti-monotone), whereas an empty *conjunction* says nothing about any clause inside it. `Clause.rows_for(pairs)` is the one place clause rows are minted (get-or-create, idempotent), shared by the pool, a fetched node and a blacklisted set.
- **EmptyClauseSet** (`core/models.py`) — a **multi-clause** conjunction the index matches nobody with: `clauses` (M2M) + `clause_key` (unique). Written by a descent probe that came back empty, read as the anti-monotone pruning rule (**a candidate is dead iff some recorded set is a subset of it**), so a conjunction is never probed twice and its supersets are never probed at all. Global, no campaign FK — same argument as `Clause`: emptiness is a fact about the provider's index, so one campaign's probes prune every campaign's lattice. The `k=1` case never lands here (that is `Clause.is_live`), and a **barren yield never does** — only emptiness prunes.
